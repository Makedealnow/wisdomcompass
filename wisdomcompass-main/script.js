...
function getPassageStudy(ref, guide, userText) {
  const base = commentaryForRef(ref);
  const lowerGuide = (guide && guide.name || "").toLowerCase();
  const isDivorce = lowerGuide.includes("divorce") || /divorce|separat|husband|wife|marriage|spouse/i.test(userText || "");
  const divorceNotes = {
    "Matthew 19:3-9": "This passage establishes Jesus' hard teaching that marriage is a covenant joined by God, not a casual agreement to dissolve. It also names sexual immorality as an exception in verse 9. Do not stop with this passage alone; continue studying 1 Corinthians 7:10-16 for abandonment, peace, and separation questions.",
    "Mark 10:2-12": "This passage reinforces the seriousness of marriage and warns against treating divorce as a simple escape from conflict. Because Mark emphasizes the covenant standard strongly, read it together with Matthew 19:9 and 1 Corinthians 7:10-16 to see the broader biblical teaching.",
    "1 Corinthians 7:10-16": "This passage adds important biblical teaching for difficult marriage situations, including separation, reconciliation, an unbelieving spouse, abandonment, and God's call to peace. It helps the user consider whether the situation involves more than ordinary conflict.",
    "Malachi 2:14-16": "This passage warns against treachery in marriage and shows that God cares deeply about covenant faithfulness. It helps examine whether betrayal, cruelty, or covenant-breaking is present rather than treating divorce as a casual preference.",
    "Ephesians 5:21-33": "This passage describes the kind of humility, sacrifice, love, and responsibility marriage is meant to reflect. It helps the user evaluate whether both spouses are pursuing the biblical pattern of love, honor, and service.",
    "Proverbs 15:1": "This passage helps when conflict, harsh speech, anger, or escalation is shaping the marriage. It does not answer divorce by itself, but it helps identify whether communication and repentance need attention.",
    "Proverbs 11:14": "This passage points the user toward wise counsel. A divorce decision should not be made alone, especially when safety, sin, children, finances, or long-term consequences are involved.",
    "James 1:5": "This passage reminds the user to seek wisdom from God before acting. It supports prayer, humility, and patience while studying the fuller biblical teaching."
  };
  if (isDivorce && divorceNotes[ref]) {
    return { ...base, relation: divorceNotes[ref] };
  }
  return base;
}

function renderPassageBlock(ref, userText = "") {
  const passage = PASSAGES[ref];
  if (!passage) return "";
  const guide = selectGuide(userText || "");
  const c = getPassageStudy(ref, guide, userText);
  ...
  return `<h4>${escapeHTML(passage.title)}</h4>
    ...
    <div class="commentary-panel simple-study-notes">
      ...
      <section class="study-note-block">
        <h5>How This Applies to Your Situation</h5>
...
function reasonForReference(ref, guide) {
  const reasons = {
    "Matthew 19:3-9": "Jesus teaches God's design for marriage and names sexual immorality as an exception.",
    "Mark 10:2-12": "Jesus reinforces the covenant seriousness of marriage and warns against casual divorce.",
    "1 Corinthians 7:10-16": "Paul addresses separation, reconciliation, abandonment by an unbelieving spouse, and peace.",
    "Malachi 2:14-16": "God confronts treachery against the wife of one's covenant.",
    "Ephesians 5:21-33": "Paul describes marriage through humility, sacrificial love, and responsibility.",
    "Proverbs 15:1": "Wisdom for conflict, speech, anger, and de-escalation.",
    "Proverbs 11:14": "Shows the importance of counsel before major decisions.",
    "James 1:5": "Encourages asking God for wisdom before acting.",
    "Luke 14:28-30": "Teaches counting the cost before a major commitment.",
    "Proverbs 22:7": "Warns about debt and obligation.",
    "Proverbs 21:5": "Commends diligent planning over haste.",
    "1 Timothy 6:6-10": "Warns about greed and discontent in money decisions.",
    "Genesis 41:33-40": "Shows wise planning, preparation, and stewardship.",
    "Psalm 90:12": "Frames time and life stages as stewardship before God.",
    "Colossians 3:23": "Calls work to be done sincerely as service to the Lord.",
    "Daniel 1:8-20": "Shows conviction and integrity under workplace pressure.",
    "Genesis 50:15-21": "Shows forgiveness without denying that real wrong occurred.",
    "Matthew 18:15-17": "Gives a process for addressing serious conflict and sin.",
    "Romans 12:18": "Calls believers to pursue peace as far as it depends on them.",
    "Galatians 5:22-23": "Shows fruit that should mark a Spirit-led decision.",
    "Psalm 119:105": "Shows Scripture as light for the next faithful step."
  };
  return reasons[ref] || "Additional passage for broader biblical context.";
}

function whatBibleSays(guide, refs, userText) {
  const isDivorce = guide.name.toLowerCase().includes("divorce") || /divorce|separat|husband|wife|marriage|spouse/i.test(userText || "");
  if (isDivorce) {
    return "Taken together, these passages teach that marriage is a serious covenant before God and should not be broken casually. Jesus points back to God's design while naming sexual immorality as an exception. Paul adds teaching about separation, reconciliation, abandonment by an unbelieving spouse, and God's call to peace. Other passages emphasize covenant faithfulness, sacrificial love, gentle speech, and wise counsel. Study the related passages before drawing conclusions from one verse alone.";
  }
  if (guide.name.toLowerCase().includes("career")) {
    return "Taken together, these passages teach that work decisions should be guided by trust in God, integrity, diligence, wise counsel, and service to the Lord. A good opportunity should be weighed not only by income or status, but also by its effect on faithfulness, family responsibility, character, and long-term fruit.";
  }
  if (guide.name.toLowerCase().includes("money")) {
    return "Taken together, these passages teach stewardship, planning, contentment, caution with debt, and the need to count the cost. Financial decisions should not be driven by fear, greed, pressure, or haste, but by wisdom, counsel, and responsibility before God.";
  }
  if (guide.name.toLowerCase().includes("retirement")) {
    return "Taken together, these passages frame retirement and later-life decisions as stewardship. Scripture emphasizes wisdom with time, planning, contentment, provision, purpose, and responsibility rather than simply comfort or escape from work.";
  }
  if (guide.name.toLowerCase().includes("relationship")) {
    return "Taken together, these passages teach truth, forgiveness, repentance, wise confrontation, peace, and Spirit-led character. They do not ignore harm or repeated wrongdoing, but they call the user to pursue wisdom rather than revenge, denial, or anger.";
  }
  return `Taken together, these passages point toward ${guide.themes.join(", ")}. Scripture calls the user to slow down, seek God, compare the relevant passages, ask for wisdom, seek counsel, and consider the likely fruit before making a major decision.`;
}
...
function guidanceToHTML(guidance, userText) {
  ...
  const buttons = refs.map((ref, idx) => `<button type="button" class="passage-link ${idx === 0 ? "selected" : ""}" data-ref="${escapeHTML(ref)}">${escapeHTML(ref)}<span>${escapeHTML(reasonForReference(ref, guide))}</span></button>`).join("");
  const stories = guide.stories.map(story => `<li>${escapeHTML(story)}</li>`).join("");
  const questions = guide.questions.map(q => `<li>${escapeHTML(q)}</li>`).join("");
  const passageHTML = passage ? `
    <div class="passage-reader" id="passageReader">
      <section class="compact-section situation-section">
        <h3>Your Situation</h3>
        <p>${escapeHTML(userText)}</p>
      </section>
      <section class="compact-section bible-says-section">
        <h3>What the Bible Says</h3>
        <p>${escapeHTML(whatBibleSays(guide, refs, userText))}</p>
      </section>
      <div class="passage-header">
        <h3>Primary Bible Passage</h3>
        <p class="microcopy">Read the full KJV passage first. Then select another passage below to study how it contributes to the full biblical picture.</p>
      </div>
      <div class="passage-display" id="passageDisplay">
        ${renderPassageBlock(firstRef, userText)}
      </div>
      <div class="passage-links" id="passageLinks"><h3>Related Bible Passages</h3>${buttons}</div>
      <section class="compact-section stories-section">
        <h3>Related Biblical Stories</h3>
        <ul>${stories}</ul>
      </section>
      <section class="compact-section next-step-section">
        <h3>Next Wise Step</h3>
        <p>${escapeHTML(guide.next)}</p>
      </section>
      <section class="compact-section questions-section">
        <h3>Questions for Reflection</h3>
        <ul>${questions}</ul>
      </section>
      <section class="compact-section disclaimer-section">
        <h3>Disclaimer</h3>
        <p>WisdomCompass provides spiritual and biblical guidance only. It is not legal, medical, mental health, financial, or emergency advice. For urgent danger, abuse, self-harm, or emergency situations, contact emergency services, a qualified professional, or trusted local help immediately.</p>
      </section>
    </div>` : "";
  return `${passageHTML}`;
}
...
function attachPassageHandlers() {
  ...
  document.querySelectorAll(".passage-link").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      display.innerHTML = renderPassageBlock(ref, situationInput ? situationInput.value : "");
      display.scrollTop = 0;
    });
  });
}
